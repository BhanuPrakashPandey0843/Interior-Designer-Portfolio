...
const skills = [
  { name: 'AutoCAD',    color: '#E51837', Icon: DraftingCompass },
  { name: 'SketchUp',   color: '#005F9E', Icon: Building        },
  { name: 'V-Ray',      color: '#85B4E3', Icon: Zap             },
  { name: 'Illustrator',color: '#FF9A00', Icon: PenTool         },
  { name: 'Photoshop',  color: '#31A8FF', Icon: ImageIcon       },
  { name: 'PowerPoint', color: '#B7472A', Icon: Presentation    },
...
const PortfolioPage = () => {
  ...
  const allPortfolioItems = [
    {
      ...
      details: {
        ...
        area: '3,000 sq ft',
        style: 'Modern Retail',
        highlights: ['Shell-inspired curves','Curated display zones','Natural materials blend','Soft lighting design','Open spatial flow','Minimal aesthetic']
      }
    },
    {
      id: 6,
      ...
      details: {
        ...
        highlights: ['Open concept design','Complete 2D documentation','Master bedroom suite','Kitchen integration','Material specifications','Lighting design']
      }
    },
    {
      id: 2,
      ...
      details: {
        ...
        highlights: ['Custom joinery','Material selection','Spatial harmony','Functional elegance','Handcrafted details','Client collaboration']
      }
    },
    {
      id: 4,
      ...
      details: {
        ...
        highlights: ['Clean lines design','Green & white palette','Textured backsplash','Open shelving','Under-cabinet lighting','Built-in appliances']
      }
    },
    {
      id: 9,
      ...
      details: {
        ...
        highlights: ['Tropical design philosophy','Heritage elements','Open floor plans','Natural light focus','Traditional Chettinad architecture','Contemporary living spaces']
      }
    }
  ];
  ...
  return (
    ...
    >
                ...
                >
                  {allPortfolioItems.map((item, index) => (
                    <button
                      key={item.id}
                      onClick={() => {
                        if (item.caseStudy) setSelectedCaseStudy(item.caseStudy);
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-8">
            {allPortfolioItems.map((project, index) => (
              <div
                key={project.id}
                onClick={() => {
                  if (project.caseStudy) setSelectedCaseStudy(project.caseStudy);
              ...
              >
                <div
                  className="relative h-40 sm:h-64 overflow-hidden bg-muted flex-shrink-0 cursor-pointer"
                  onClick={(e) => { e.stopPropagation(); setFullscreenImage(project.image); }}
                >
                  <img src={project.image} alt={project.title} className="w-full h-full object-cover group-hover:scale-110 transition-transform duration-500" />
                  <div className="absolute inset-0 bg-black/40 group-hover:bg-black/50 transition-colors"></div>
                </div>
                <div className="p-3 sm:p-6 bg-card border border-border/50 flex-1 flex flex-col">
                  <h3 className="font-serif text-base sm:text-xl font-bold mb-2 group-hover:text-primary transition-colors line-clamp-2">{project.title}</h3>
                  <p className="text-xs sm:text-sm opacity-75 line-clamp-2 mb-3 flex-1">{project.description}</p>
                  <div className="inline-block text-primary text-xs sm:text-sm font-medium group-hover:translate-x-1 transition-transform">View Details →</div>
                </div>
              </div>
            ))}
          </div>
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 sm:gap-8 max-w-5xl mx-auto">
            <div className="card-hover rounded-lg overflow-hidden bg-card border border-border/50 smooth-hover flex flex-col h-full">
              <div className="relative h-56 sm:h-96 overflow-hidden bg-muted flex-shrink-0 cursor-pointer group" onClick={() => setFullscreenImage('/kingston-certificate.jpg')}>
                <img src="/kingston-certificate.jpg" alt="Kingston University Certificate" className="w-full h-full object-cover hover:scale-105 transition-transform duration-500" />
                <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-colors flex items-center justify-center">
                  <div className="opacity-0 group-hover:opacity-100 transition-opacity bg-black/60 text-white px-4 py-2 rounded-lg text-sm font-medium">Click to expand</div>
                </div>
              </div>
              <div className="p-4 sm:p-8 flex flex-col flex-1">
                <h3 className="font-serif text-lg sm:text-2xl font-bold mb-2 sm:mb-3">Kingston University</h3>
                <p className="text-sm sm:text-base mb-2 font-medium text-primary">Bachelor of Arts with Honours (Third Class)</p>
                <p className="text-xs sm:text-sm mb-2 sm:mb-3 text-primary">Interior Architecture & Design</p>
                <p className="opacity-75 text-xs sm:text-sm flex-1">Advanced degree programme in Interior Architecture and Design in collaboration with the Indian Institute of Art and Design (IIAD). Developed expertise in spatial design, architectural visualization, and comprehensive project management. Graduated June 2025.</p>
              </div>
            </div>
            <div className="card-hover rounded-lg overflow-hidden bg-card border border-border/50 smooth-hover flex flex-col h-full">
              <div className="relative h-56 sm:h-96 overflow-hidden bg-muted flex-shrink-0 cursor-pointer group" onClick={() => setFullscreenImage('https://hebbkx1anhila5yf.public.blob.vercel-storage.com/cetificate_sres-gJ6l7Ys4WggZ3hQlWv7hht6RYzVHel.jpg')}>
                <img src="https://hebbkx1anhila5yf.public.blob.vercel-storage.com/cetificate_sres-gJ6l7Ys4WggZ3hQlWv7hht6RYzVHel.jpg" alt="IIAD Certificate" className="w-full h-full object-cover hover:scale-105 transition-transform duration-500" />
                <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-colors flex items-center justify-center">
                  <div className="opacity-0 group-hover:opacity-100 transition-opacity bg-black/60 text-white px-4 py-2 rounded-lg text-sm font-medium">Click to expand</div>
                </div>
              </div>
              <div className="p-4 sm:p-8 flex flex-col flex-1">
                <h3 className="font-serif text-lg sm:text-2xl font-bold mb-2 sm:mb-3">Indian Institute of Art & Design</h3>
                <p className="text-sm sm:text-base mb-2 font-medium text-primary">Diploma in Interior Design & Graphic Design</p>
                <p className="opacity-75 text-xs sm:text-sm flex-1">Specialized training in contemporary interior design, spatial planning, graphic design, and visual communication. Completed comprehensive coursework in design principles, material selection, colour theory, and professional project delivery with hands-on experience.</p>
              </div>
            </div>
          </div>
        </div>
          ...
          <div className="bg-muted/30 rounded-lg p-6 sm:p-12 md:p-20 flex flex-col md:flex-row items-center md:items-start justify-between gap-6 sm:gap-12 smooth-hover">
            ...
            <div className="text-center md:text-left flex-1">
              ...
              <p className="text-xs sm:text-lg mb-2 sm:mb-3">
                ...
                <a href="mailto:sresthajyoti@gmail.com" className="text-primary hover:text-accent transition-colors font-medium break-all text-xs sm:text-base">sresthajyoti@gmail.com</a>
              </p>
              <p className="text-xs sm:text-lg mb-2 sm:mb-3">
                <span className="text-foreground font-medium">Phone: </span>
                <a href="tel:+917033661527" className="text-primary font-medium text-xs sm:text-base hover:text-accent transition-colors">+91 7033661527</a>
              </p>
              <p className="text-xs sm:text-lg">
                <span className="text-foreground font-medium">LinkedIn: </span>
                <a href="https://www.linkedin.com/in/srestha-jyoti-295b71218/" target="_blank" rel="noopener noreferrer" className="text-primary font-medium text-xs sm:text-base hover:text-accent transition-colors inline-flex items-center gap-1.5 group">
      ...
      {selectedCaseStudy && (
        <div className="fixed inset-0 bg-black/60 z-50 flex items-start justify-center p-3 sm:p-4 overflow-y-auto modal-backdrop pt-16 sm:pt-20">
          <div className="bg-card rounded-2xl max-w-6xl w-full modal-content case-study-modal border border-border p-4 sm:p-8 md:p-12 my-10 sm:my-20">
            <button onClick={() => setSelectedCaseStudy(null)} className="absolute top-3 right-3 sm:top-4 sm:right-4 p-2 bg-muted rounded-full hover:bg-primary hover:text-primary-foreground transition-all z-10 flex-shrink-0" aria-label="Close modal">
              <X size={20} />
            </button>
        ...
        <div className="fixed inset-0 bg-black/60 z-50 flex items-center justify-center p-3 sm:p-4 modal-backdrop">
          <div className="bg-card rounded-2xl max-w-3xl w-full max-h-[90vh] overflow-y-auto modal-content border border-border">
            ...
            <div className="relative h-48 sm:h-64 overflow-hidden cursor-pointer group" onClick={() => setFullscreenImage(selectedProject.image)}>
              <img src={selectedProject.image} alt={selectedProject.title} className="w-full h-full object-cover group-hover:scale-105 transition-transform duration-300" />
              <div className="absolute inset-0 bg-black/0 group-hover:bg-black/20 transition-colors flex items-center justify-center">
                <div className="opacity-0 group-hover:opacity-100 transition-opacity bg-black/60 text-white px-4 py-2 rounded-lg text-sm font-medium">Click to expand</div>
              </div>
            </div>
            <div className="p-4 sm:p-8 space-y-4 sm:space-y-6">
              <div>
                <h2 className="font-serif text-xl sm:text-3xl font-bold mb-2">{selectedProject.title}</h2>
                <p className="text-xs sm:text-base leading-relaxed opacity-90">{selectedProject.description}</p>
              </div>
              <div className="grid grid-cols-2 gap-3 sm:gap-6 border-t border-border pt-4 sm:pt-6">
                <div><p className="text-xs opacity-60 font-medium">CLIENT</p><p className="text-xs sm:text-lg font-semibold">{selectedProject.details.client}</p></div>
                <div><p className="text-xs opacity-60 font-medium">YEAR</p><p className="text-xs sm:text-lg font-semibold">{selectedProject.details.year}</p></div>
                <div><p className="text-xs opacity-60 font-medium">AREA</p><p className="text-xs sm:text-lg font-semibold">{selectedProject.details.area}</p></div>
                <div><p className="text-xs opacity-60 font-medium">STYLE</p><p className="text-xs sm:text-lg font-semibold">{selectedProject.details.style}</p></div>
              </div>
      ...
      {fullscreenImage && (
        <div className="fixed inset-0 bg-black/95 z-[9999] flex items-center justify-center p-3 sm:p-4" onClick={() => setFullscreenImage(null)}>
          <button onClick={(e) => { e.stopPropagation(); setFullscreenImage(null); }} className="absolute top-4 right-4 z-[10000] p-2 sm:p-3 bg-white/20 hover:bg-white/40 rounded-full transition-all" aria-label="Close fullscreen">
            <X size={24} className="text-white" />
          </button>
          <div className="max-w-[90vw] max-h-[90vh] flex items-center justify-center" onClick={(e) => e.stopPropagation()}>
            <img src={fullscreenImage} alt="Fullscreen view" className="w-full h-full object-contain max-w-full max-h-full" />
          </div>
